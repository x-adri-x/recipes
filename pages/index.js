...
const Home = () => {
  ...
  const fetchData = async() => {
      const result = await fetch(`${process.env.CONTENTFUL_CDN}/spaces/${process.env.SPACE_ID}/environments/master/entries`
  ...
  const addTodo = async(newTodo) => {
    ...
    if(result) {
      ...
      .catch(error => console.log(`An error has occured: ${error}`))
    }else{console.log('Adding of a new recipe has failed massively. Do something!')}
    setName('')
    setUrl('')
  }
  ...
  const deleteTodo = async(e) => {
    ...
    const result = await fetch(`${process.env.HOST}/spaces/${process.env.SPACE_ID}/environments/master/entries/${entry}/published`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${process.env.CMA_ACCESS_TOKEN}`, 
      }
      ...
      <div className = {styles.home}>
        <div className = {styles.container}>
          ...
          <div className = {styles.input_container}>
              ...
              <button className = {styles.addButton} onClick = {() => mutationAddEntry.mutate({"fields":{"name":{"en-US":name}, "url":{"en-US":url}}})}><IoIosAdd size = '2em'/></button>
          </div>
